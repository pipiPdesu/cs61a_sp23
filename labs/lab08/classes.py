...
class TACard(Card):
    ...
    def effect(self, opponent_card, player, opponent, arg=None):
        ...
        for i in range(len(player.hand)):
            ...
            elif player.hand[i].attack > best_card.attack and player.hand[i].cardtype == 'Staff':#一定要是普通卡
                print("DEBUG:", player.hand[i].attack, best_card.attack)
                best_card = player.hand[i]
        try:
            print("DEBUG:", best_card.attack, best_card.name)
        except:
            print("DEBUG: No best card")
        if best_card:
