...
class Person:
    ...
    def __init__(self, name):
        self.name = name
        self.rep = 'I squirreled it away before it could catch on fire.'
        "*** YOUR CODE HERE ***"
    ...
    def play_round(self, starter, cards):
        ...
        for who, card in cards:
            try:
                ...
                print(e)
        if r.winner != None and r.is_complete():#小改一句没问题吧
...
class Round:
    ...
    def play(self, who, card):
        ...
        if card >= self.highest and not self.is_complete():
            self.highest = card
            self.winner = who
